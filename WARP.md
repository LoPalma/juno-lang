...
# Build the fast standalone compiler (recommended)
mvn compile && cd target/classes && jar -cfm ../../junoc-standalone.jar MANIFEST.MF com/
...
### Running the Compiler (Fast Method - Recommended)
```bash
# Compile a Juno source file (0.1-0.2 seconds)
./junoc myprogram.juno

# Generate both bytecode and Jasmin assembly for debugging
./junoc -j myprogram.juno
...
### Running the Compiler (Legacy Maven Method - Slow)
```bash
# Compile using Maven exec plugin (2-3 seconds - slow due to Maven startup)
...
### File Extensions
Source files can use either:
- `.juno` extension (recommended - Juno language)
- `.cl` extension (legacy - C-Like language)

Both extensions are fully supported by the `junoc` compiler.
...
### Compilation Speed
- **Fast**: `./junoc` standalone executable (~0.15-0.20 seconds)
- **Slow**: `mvn exec:java` Maven method (~2-3 seconds due to JVM startup overhead)

### Debugging Tools
- **Jasmin Assembly**: Use `-j` flag to generate human-readable `.j` files
- **Bytecode Analysis**: Use `javap -c <classname>` to inspect generated bytecode
- **Error Messages**: Clear error reporting with line/column information

### File Organization
- **Source files**: `.juno` or `.cl` files in project directory
- **Bytecode output**: `.class` files in same directory as source
- **Jasmin assembly**: `.j` files (when using `-j` flag) for debugging
- **Compiler**: `junoc` executable wrapper + `junoc-standalone.jar`
